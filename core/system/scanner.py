...
def scan_system() -> dict:
    """Collect the complete system hardware and software profile.

    Gathers OS details, CPU specs, memory topology, GPU configurations,
    and storage disk capacities into a structured dictionary.

    Returns:
        dict: Machine-readable system hardware and software profile.

    Raises:
        Exception: If an unexpected error occurs during hardware scanning.
    """
    write_log(
        level="INFO",
        component=COMPONENT,
    ...
    try:
        ...
        profile = {
            ...
            "storage": [
                {
                    "drive": drive["drive"],
                    "total_gb": round(bytes_to_gb(drive["total"]), 2),
                    "used_gb": round(bytes_to_gb(drive["used"]), 2),
                    "free_gb": round(bytes_to_gb(drive["free"]), 2),
                }
