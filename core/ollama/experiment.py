...
def _generate(
    ...
    """
    Run one temporary model configuration against one prompt.

    The configuration is applied only to this request.
    """
...
def run_test(
    ...
    The model is checked and loaded before each prompt.
    Model loading is not included in test timing or results.
    """
    ...
    for index, prompt in enumerate(
        ...
        try:
            ...
            output_duration_ns = response.get(
                "eval_duration",
                0,
            )
...
def compare_tests(
    ...
    """
    Run the same prompts against multiple temporary
